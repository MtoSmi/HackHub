...
import it.unicam.cs.ids.hackhub.controller.SubmissionIterfaceController;
...
public class AggiungiSottomissioniTest {
    ...
    private SubmissionIterfaceController SubmissionController;
    ...
    void setUp() {
        ...
        SubmissionController = new SubmissionIterfaceController(new SubmissionService(new SubmissionValidator(), HackathonRepository));
        user.setId(1L);
    ...
    public void testAggiungiSottomissioneValida() {
        ...
        Assertions.assertNotNull(submissionResult, "La sottomissione creata non dovrebbe essere null");
        Assertions.assertEquals(submissionRequester.getTitle(), submissionResult.getTitle(), "Il titolo della sottomissione creata dovrebbe corrispondere a quello della richiesta");
        Assertions.assertEquals(submissionRequester.getDescription(), submissionResult.getDescription(), "La descrizione della sottomissione creata dovrebbe corrispondere a quella della richiesta");
        Assertions.assertEquals(submissionRequester.getStartDate(), submissionResult.getStartDate(), "La data di inizio della sottomissione creata dovrebbe corrispondere a quella della richiesta");
        Assertions.assertEquals(submissionRequester.getEndDate(), submissionResult.getEndDate(), "La data di fine della sottomissione creata dovrebbe corrispondere a quella della richiesta");
    }
